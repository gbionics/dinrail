...
TEST_CASE("dinrail::VectorProxy")
{
    ...
    SECTION("Copy")
    {
        ...
        copiedIn.resize(5);
        dinrail::VectorProxy<double> containerToBeCopied(copiedIn); // copied in is automatically
                                                                    // casted to a span
    ...
    SECTION("Create pointer")
    {
        ...
        container_ptr
            = dinrail::make_vector_ptr(std::span(container.data(), container.size()), resizeLambda);
