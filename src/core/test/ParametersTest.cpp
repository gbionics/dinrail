...
TEST_CASE("Parameters supports BLF-style generic vector set and get", "[Parameters]")
{
    ...
    params.setParameter("ints", sourceInt);
    params.setParameter("doubles", sourceDouble);
    params.setParameter("strings", sourceString);
    params.setParameter("durations", sourceDuration);
    ...
    REQUIRE(params.getParameter("ints", outInt));
    REQUIRE(params.getParameter("doubles", outDouble));
    REQUIRE(params.getParameter("strings", outString));
    REQUIRE(params.getParameter("durations", outDuration));
    ...
    // If we pass a non-resizable vector with the wrong size, getParameter should fail
    std::array<int, 2> fixedInt{0, 0};
    REQUIRE_FALSE(params.getParameter("ints", fixedInt));

    // If we pass a non-resizable vector with the correct size, getParameter should succeed
    std::array<int, 3> fixedIntCorrectSize{0, 0, 0};
    REQUIRE(params.getParameter("ints", fixedIntCorrectSize));
    REQUIRE(fixedIntCorrectSize.size() == sourceInt.size());
    for (std::size_t i = 0; i < fixedIntCorrectSize.size(); ++i)
    {
        REQUIRE(fixedIntCorrectSize[i] == sourceInt[i]);
    }
}
