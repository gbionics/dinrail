...
/**
 * @brief Interface to set all impedance control setpoints (position, velocity, torque, stiffness,
 * damping) at once.
 *
 * This interface permits to set all impedance control setpoints for multiple axes in a single
 * method call, ensuring that a set of parameters for impedance control is applied simultaneously.
 *
 ...
 *
 * The unit of the position, velocity, torque, stiffness and damping setpoints is the same as the
 * one used by the:
...
public:
     ...
     */
    virtual bool
    setSetPoint(int j, double pos, double vel, double torque, double stiffness, double damping) = 0;
