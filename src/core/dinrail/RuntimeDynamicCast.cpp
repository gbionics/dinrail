...
void* runtimeDynamicCast(const PolymorphicView& source, const std::type_info& testedType) noexcept
{
    ...
    // __RTDynamicCast expects the subobject whose first word is the RTTI vfptr;
    // vfDelta recovers it from the source pointer. For a class type, the address
    // of its std::type_info is the TypeDescriptor consumed by the runtime.
    void* const rttiPtr = static_cast<void*>(static_cast<char*>(basePtr) + vfDelta);
