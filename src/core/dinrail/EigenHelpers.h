...
template <typename Class> auto to_eigen(Class& input)
{
    ...
    return Eigen::Map<Eigen::Matrix<value_type, Eigen::Dynamic, 1>>(genericVector.data(),
                                                                    genericVector.size());
}
...
template <typename Class> auto to_eigen(const Class& input)
{
    ...
    auto genericVector = make_vector(input);
    return Eigen::Map<const Eigen::Matrix<value_type, Eigen::Dynamic, 1>>(genericVector.data(),
                                                                          genericVector.size());
}
