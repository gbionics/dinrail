 ...
 * 
 * This class was based on the BipedalLocomotion::GenericVector class in March 2026, with the following modifications:
 * - It has been renamed to GenericVector and moved in the dinrail namespace,
 * - It has been migrated from std::span to std::span,
 * - It has been migrated to not include Eigen headers, while retaining the possibility to be initialized from Eigen vectors.
 * 
...
public:
     ...
     *
     * Some notes:
     * - vector_element_type has a different name than the corresponding one in std::span to avoid some compilation issues.
    ...
    using index_type = typename std::span<T>::index_type;
    ...
    using const_reverse_iterator = typename std::span<T>::const_reverse_iterator;
...
public:
     ...
     */
    Vector(std::span<T> span, resize_function_type resizeLambda)
    {
        m_span = span;
    ...
    bool clone(const Vector<T>& other)
    {
        if (size() != other.size())
        {
            ...
            {
                std::cerr << "[GenericVector] Failed to resize. Copy aborted" << std::endl;
                return false;
            }
        }
     ...
     * @warning It performs memory allocation if this is resizable and the sizes are different.
     */
    Vector<T>& operator=(std::span<T> other)
    {
        bool ok = clone(other);
        assert(ok);
        unused(ok);
...
/**
 * is_vector is a utility metafunction used to check if T is a GenericVector.
 */
template <typename T>
...
/**
 * is_span_constructible is a utility metafunction to check if std::span is constructible given a reference to Class.
 */
...
/**
 * is_vector_constructible is a utility metafunction to check if GenericVector is constructible given a type T.
 */
 ...
 * If not, this specialization is not used (SFINAE). Otherwise, given the vector type, it checks if a std::span is construbile
...
/**
 * is_vector_ref_constructible is a utility metafunction to check if GenericVector::Ref is constructible given a type T and a Container.
 */
...
/**
 * is_vector_constructible is a utility metafunction to check if GenericVector::Ref is constructible given a type T and a Container.
 * This specialization first checks if a GenericVector can be constructed given the Container.
...
/**
 * @brief Utility function to create a GenericVector from a reference to another vector.
 * @param input The refence to an existing vector.
 * @param mode The resize mode. By default the output Vector is <code>Fixed<\code>.
 * @returns A GenericVector pointing to the input vector.
 *
 * @warning The input object from which GenericVector has been initialized should not be deallocated before it.
 * This would invalidate the pointer inside GenericVector.
 */
template<typename Class>
...
make_vector(Class& input, VectorResizeMode mode = VectorResizeMode::Fixed)
{
    ...
    else
    {
        if (mode == VectorResizeMode::Resizable)
        {
            std::cerr << "[GenericVector] " << type_name<Class>()
...
make_vector(const Class& input, VectorResizeMode mode = VectorResizeMode::Fixed)
{
    ...
    if (mode == VectorResizeMode::Resizable)
    {
        std::cerr << "[GenericVector] The input type is const. Returning a non-resizable container." << std::endl;
    }
...
/**
 * @brief Utility function to create a GenericVector_ptr from a reference to another vector.
 * @param input The refence to an existing vector.
 * @param mode The resize mode. By default the output Vector is <code>Fixed<\code>.
 * @returns A GenericVector_ptr.
 *
 * @warning The input object from which the underlying GenericVector has been initialized should not be deallocated before it.
 * This would invalidate the pointer inside GenericVector.
 */
template<typename Class>
...
template<typename T>
Vector_ptr<T>
make_vector_ptr(std::span<T> span, typename Vector<T>::resize_function_type resizeLambda)
{
 ...
 * is that custom vectors (all those supported by GenericVector) can be implicitly casted to Ref.
 ...
 */
template <typename T>
class BipedalLocomotion::GenericVector<T>::Ref : public BipedalLocomotion::GenericVector<T>
{
public:
     ...
     */
    Ref(BipedalLocomotion::GenericVector<T>::Ref& other)
    {
        m_span = other.m_span;
        m_resizeLambda = other.m_resizeLambda;
    }
    ...
    /**
     * @brief Constructor from a GenericVector<T>&
     * @param other The input vector from which the context is copied
     */
    Ref(BipedalLocomotion::GenericVector<T>& other)
    {
        m_span = other.m_span;
        m_resizeLambda = other.m_resizeLambda;
    }
     ...
     * This is used if:
     * - the input container is not a GenericVector, to avoid ambiguities with other constructors
     * - the input container is not a string. This allows using Ref and string with overloaded methods.
     * - a GenericVector<T>::Ref can be constructed from the Container
     * - T is not const
   ...
   {
       return static_cast<BipedalLocomotion::GenericVector<T>&>(*this) = other;
   }
