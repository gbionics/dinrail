...
bool StateExtendedInputPort::getLastSingle(
    int j, int field, double* data, Stamp& stamp, double& localArrivalTime)
{
    mutex.lock();
    bool ret = valid;
    if (ret)
...
bool StateExtendedInputPort::getLastSingle(
    int j, int field, int* data, Stamp& stamp, double& localArrivalTime)
{
    mutex.lock();
    bool ret = valid;
    if (ret)
                                           ...
                                           double& localArrivalTime)
{
    ...
    if (ret)
    {
        ...
        case VOCAB_ENCODERS:
            ret = last.jointPosition_isValid;
            memcpy(data,
                   last.jointPosition.data(),
                   last.jointPosition.size() * last.jointPosition.getElementSize());
            break;

        case VOCAB_ENCODER_SPEEDS:
            ret = last.jointVelocity_isValid;
            memcpy(data,
                   last.jointVelocity.data(),
                   last.jointVelocity.size() * last.jointVelocity.getElementSize());
            break;

        case VOCAB_ENCODER_ACCELERATIONS:
            ret = last.jointAcceleration_isValid;
            memcpy(data,
                   last.jointAcceleration.data(),
                   last.jointAcceleration.size() * last.jointAcceleration.getElementSize());
            break;

        case VOCAB_MOTOR_ENCODERS:
            ret = last.motorPosition_isValid;
            memcpy(data,
                   last.motorPosition.data(),
                   last.motorPosition.size() * last.motorPosition.getElementSize());
            break;

        case VOCAB_MOTOR_ENCODER_SPEEDS:
            ret = last.motorVelocity_isValid;
            memcpy(data,
                   last.motorVelocity.data(),
                   last.motorVelocity.size() * last.motorVelocity.getElementSize());
            break;

        case VOCAB_MOTOR_ENCODER_ACCELERATIONS:
            ret = last.motorAcceleration_isValid;
            memcpy(data,
                   last.motorAcceleration.data(),
                   last.motorAcceleration.size() * last.motorAcceleration.getElementSize());
            break;
        ...
        case VOCAB_PWMCONTROL_PWM_OUTPUTS:
            ret = last.pwmDutycycle_isValid;
            memcpy(data,
                   last.pwmDutycycle.data(),
                   last.pwmDutycycle.size() * last.pwmDutycycle.getElementSize());
            break;
        ...
        case VOCAB_TEMPERATURES:
            ...
            memcpy(data,
                   ...
                   last.temperature.size() * last.temperature.getElementSize());
            break;
                                           ...
                                           double& localArrivalTime)
{
    ...
    if (ret)
    {
        ...
        case VOCAB_CM_CONTROL_MODES:
            ret = last.controlMode_isValid;
            memcpy(data,
                   last.controlMode.data(),
                   last.controlMode.size() * last.controlMode.getElementSize());
            break;

        case VOCAB_INTERACTION_MODES:
            ret = last.interactionMode_isValid;
            memcpy(data,
                   last.interactionMode.data(),
                   last.interactionMode.size() * last.interactionMode.getElementSize());
            break;
