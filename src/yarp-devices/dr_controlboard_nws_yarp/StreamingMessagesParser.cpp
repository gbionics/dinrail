...
bool StreamingMessagesParser::initialize()
{
    ...
    if (stream_IVel)
    {
        stream_IVel->getAxes(&stream_nJoints);
        return true;
    }
    if (stream_ITorque)
    {
        stream_ITorque->getAxes(&stream_nJoints);
        return true;
    }
    if (stream_IAxis)
    {
        stream_IAxis->getAxes(&stream_nJoints);
        return true;
    }
...
void StreamingMessagesParser::onRead(CommandMessage& v)
{
    ...
    controlBoardLogger().trace("Received command %s, %s\n",
                               b.toString().c_str(),
                               cmdVector.toString().c_str());
    ...
    if (cmdVector.size() > maxCommandVectorSize)
    {
        ...
        controlBoardLogger().error("Received command vector with number of elements bigger than "
                                   "axis controlled by "
                                   "this wrapper (cmd: %s requested jnts: %zu received jnts: "
                                   "%zu)\n",
                                   str.c_str(),
                                   maxCommandVectorSize,
                                   cmdVector.size());
        return;
    }
    if (cmdVector.data() == nullptr)
    {
        controlBoardLogger().error("Received null command vector");
        return;
    }
    ...
    case VOCAB_PWMCONTROL_INTERFACE: {
        ...
        case VOCAB_PWMCONTROL_REF_PWM: {
            if (stream_IPWM)
            {
                ...
                if (!ok)
                {
                    controlBoardLogger().error("Errors while trying to command an pwm message");
                }
            } else
            {
                controlBoardLogger().error("PWM interface not valid");
            }
        }
        break;
    ...
    case VOCAB_CURRENTCONTROL_INTERFACE: {
        ...
        case VOCAB_CURRENT_REF: {
            if (stream_ICurrent)
            {
                ...
                if (!ok)
                {
                    controlBoardLogger().error("Errors while trying to command a streaming current "
                                               "message on single "
                                               "joint\n");
                }
            }
        ...
        case VOCAB_CURRENT_REF_GROUP: {
            if (stream_ICurrent)
            {
                ...
                if (jlut == nullptr)
                {
                    controlBoardLogger().error("Received malformed VOCAB_CURRENT_REF_GROUP: joint "
                                               "selection is not a "
                                               "list (%s)\n",
                                               b.toString().c_str());
                    break;
                ...
                {
                    controlBoardLogger().error("Received VOCAB_CURRENT_REF_GROUP size of joints "
                                               "vector or currents "
        ...
        default: {
            std::string str = yarp::os::Vocab32::decode(b.get(0).asVocab32());
            controlBoardLogger().error("Unrecognized message while receiving on command port "
                                       "(%s)\n",
                                       str.c_str());
        }
        break;
    ...
    case VOCAB_POSITION_MODE: {
        controlBoardLogger().error("Received VOCAB_POSITION_MODE this is an send invalid message "
                                   "on streaming port");
        break;
    }
    ...
    case VOCAB_POSITION_MOVES: {
        if (stream_IPosCtrl)
        {
            ...
            if (!ok)
            {
                controlBoardLogger().error("Errors while trying to start a position move");
            }
        }
    }
    break;
    ...
    case VOCAB_VELOCITY_MODE: {
        controlBoardLogger().error("Received VOCAB_VELOCITY_MODE this is an send invalid message "
                                   "on streaming port");
        break;
    }
    ...
    case VOCAB_VELOCITY_MOVE: {
        if (stream_IVel)
        {
            ...
            if (!ok)
            {
                controlBoardLogger().error("Errors while trying to start a velocity move");
            }
        }
    }
    break;
    ...
    case VOCAB_TORQUES_DIRECT_GROUP: {
        if (stream_ITorque)
        {
            ...
            if (jlut == nullptr)
            {
                controlBoardLogger().error("Received malformed VOCAB_TORQUES_DIRECT_GROUP: joint "
                                           "selection is not a "
                                           "list (%s)\n",
                                           b.toString().c_str());
                break;
            ...
            if ((static_cast<int>(jlut->size()) != n_joints)
                || (static_cast<int>(cmdVector.size()) != n_joints))
            {
                controlBoardLogger().error("Received VOCAB_TORQUES_DIRECT_GROUP size of joints "
                                           "vector or torques "
                                           "vector does not match the selected joint number\n");
                break;
            }
    ...
    case dinrail::VOCAB_DINRAIL_IMPEDANCE_ALL_SETPOINTS: {
        ...
        {
            controlBoardLogger().error("Received unsupported dinrail impedance-all-setpoints "
                                       "method on streaming "
                                       "port");
            break;
        }
        ...
        if (b.size() == 3)
        {
            if (cmdVector.size() != 5)
            {
                controlBoardLogger().error("Received invalid single-joint impedance-all-setpoints "
                                           "payload size %zu",
                                           cmdVector.size());
                break;
            }
            ...
            if (stream_IImpedanceAllSetPointsControl)
            {
                ok = stream_IImpedanceAllSetPointsControl->setSetPoint(joint,
                                                                       ...
                                                                       cmdVector[3],
                                                                       cmdVector[4]);
            } else if (stream_emulateImpedanceAllSetPointsControl && stream_IPosDirect
                       && stream_IVel && stream_ITorque && stream_IImpedance)
            {
        ...
        if (b.size() == 4)
        {
            ...
            {
                controlBoardLogger().error("Received invalid impedance-all-setpoints group "
                                           "payload\n");
                break;
            }
            ...
            dinrail::VectorProxy<const double>::Ref posRef(
                std::span<const double>(raw, static_cast<size_t>(n_joints)));
            dinrail::VectorProxy<const double>::Ref velRef(
                std::span<const double>(raw + n_joints, static_cast<size_t>(n_joints)));
            dinrail::VectorProxy<const double>::Ref torqueRef(
                std::span<const double>(raw + 2 * n_joints, static_cast<size_t>(n_joints)));
            dinrail::VectorProxy<const double>::Ref stiffnessRef(
                std::span<const double>(raw + 3 * n_joints, static_cast<size_t>(n_joints)));
            dinrail::VectorProxy<const double>::Ref dampingRef(
                std::span<const double>(raw + 4 * n_joints, static_cast<size_t>(n_joints)));
                       ...
                       && stream_IVel && stream_ITorque && stream_IImpedance)
            {
                ...
                for (int i = 0; i < n_joints; i++)
                {
                    ...
                    setPosOk
                        = setPosOk
                          && stream_IPosDirect->setPosition(joint, posRef[static_cast<size_t>(i)]);
                    setVelOk = setVelOk
                               && stream_IVel->velocityMove(joint, velRef[static_cast<size_t>(i)]);
                    setTorqueOk
                        = setTorqueOk
                          && stream_ITorque->setRefTorque(joint, torqueRef[static_cast<size_t>(i)]);
                    setImpOk
                        = setImpOk
                          && stream_IImpedance->setImpedance(joint,
        ...
        if (b.size() == 2)
        {
            ...
            {
                controlBoardLogger().error("Received invalid full-joint impedance-all-setpoints "
                                           "payload size %zu",
                                           cmdVector.size());
                break;
            }
            ...
            const double* raw = cmdVector.data();
            dinrail::VectorProxy<const double>::Ref posRef(
                std::span<const double>(raw, static_cast<size_t>(stream_nJoints)));
            dinrail::VectorProxy<const double>::Ref velRef(
                std::span<const double>(raw + stream_nJoints, static_cast<size_t>(stream_nJoints)));
            dinrail::VectorProxy<const double>::Ref torqueRef(
                std::span<const double>(raw + 2 * stream_nJoints,
                                        static_cast<size_t>(stream_nJoints)));
            dinrail::VectorProxy<const double>::Ref stiffnessRef(
                std::span<const double>(raw + 3 * stream_nJoints,
                                        static_cast<size_t>(stream_nJoints)));
            dinrail::VectorProxy<const double>::Ref dampingRef(
                std::span<const double>(raw + 4 * stream_nJoints,
                                        static_cast<size_t>(stream_nJoints)));
                       ...
                       && stream_IVel && stream_ITorque && stream_IImpedance)
            {
                ...
                {
                    setPosOk = setPosOk
                               && stream_IPosDirect->setPosition(i, posRef[static_cast<size_t>(i)]);
                    setVelOk = setVelOk
                               && stream_IVel->velocityMove(i, velRef[static_cast<size_t>(i)]);
                    setTorqueOk
                        = setTorqueOk
                          && stream_ITorque->setRefTorque(i, torqueRef[static_cast<size_t>(i)]);
                    setImpOk
                        = setImpOk
                          && stream_IImpedance->setImpedance(i,
                                                             stiffnessRef[static_cast<size_t>(i)],
                                                             dampingRef[static_cast<size_t>(i)]);
                }
                ok = setPosOk && setVelOk && setTorqueOk && setImpOk;
        ...
        controlBoardLogger().error("Received malformed dinrail impedance-all-setpoints command "
                                   "header");
    }
    break;
    ...
    case VOCAB_POSITION_DIRECT_GROUP: {
        if (stream_IPosDirect)
        {
            ...
            if (jlut == nullptr)
            {
                controlBoardLogger().error("Received malformed VOCAB_POSITION_DIRECT_GROUP: joint "
                                           "selection is not a "
                                           "list (%s)\n",
                                           b.toString().c_str());
                break;
            ...
            if ((static_cast<int>(jlut->size()) != n_joints)
                || (static_cast<int>(cmdVector.size()) != n_joints))
            {
                controlBoardLogger().error("Received VOCAB_POSITION_DIRECT_GROUP size of joints "
                                           "vector or positions "
                                           "vector does not match the selected joint number\n");
                break;
            }
    ...
    case VOCAB_VELOCITY_MOVE_GROUP: {
        if (stream_IVel)
        {
            ...
            if (jlut == nullptr)
            {
                controlBoardLogger().error("Received malformed VOCAB_VELOCITY_MOVE_GROUP: joint "
                                           "selection is not a "
                                           "list (%s)\n",
                                           b.toString().c_str());
                break;
            ...
            if ((static_cast<int>(jlut->size()) != n_joints)
                || (static_cast<int>(cmdVector.size()) != n_joints))
            {
                controlBoardLogger().error("Received VOCAB_VELOCITY_MOVE_GROUP size of joints "
                                           "vector or positions "
                                           "vector does not match the selected joint number\n");
                break;
            }
    ...
    default: {
        std::string str = yarp::os::Vocab32::decode(b.get(0).asVocab32());
        controlBoardLogger().error("Unrecognized message while receiving on command port (%s)\n",
                                   str.c_str());
    }
    break;
    }
