...
{
    inline void exec_iImpedanceAllSetPointsControl_test_1(dinrail::IImpedanceAllSetPointsControl* iimpAll,
                                                          yarp::dev::IAxisInfo* iinfo)
    {
...

        {
            ...
            REQUIRE(waitUntil([&]() {
                return iimpAll->getSetPoint(0, pos, vel, torque, stiffness, damping)
                       && closeEnough(pos, 1.0)
                       && closeEnough(vel, 2.0)
                       && closeEnough(torque, 3.0)
                       && closeEnough(stiffness, 4.0)
                       && closeEnough(damping, 5.0);
            }));
            CHECK(pos == 1.0);
            CHECK(vel == 2.0);
            CHECK(torque == 3.0);
        ...
        if (axes > 1) {
            ...
            REQUIRE(waitUntil([&]() {
                return iimpAll->getSetPoints(joints,
                                             gotPos,
                                             gotVel,
                                             gotTorque,
                                             gotStiffness,
                                             gotDamping)
                       && closeEnough(gotPos[0], 10.0)
                       && closeEnough(gotPos[1], 11.0)
                       && closeEnough(gotVel[0], 12.0)
                       && closeEnough(gotVel[1], 13.0)
                       && closeEnough(gotTorque[0], 14.0)
                       && closeEnough(gotTorque[1], 15.0)
                       && closeEnough(gotStiffness[0], 16.0)
                       && closeEnough(gotStiffness[1], 17.0)
                       && closeEnough(gotDamping[0], 18.0)
                       && closeEnough(gotDamping[1], 19.0);
            }));
            CHECK(gotPos[0] == 10.0);
            CHECK(gotPos[1] == 11.0);
            CHECK(gotVel[0] == 12.0);
...

        {
            ...
            REQUIRE(waitUntil([&]() {
                ...
                for (int j = 0; j < axes; ++j) {
                    ...
                    if (!closeEnough(gotPos[idx], 20.0)
                        || !closeEnough(gotVel[idx], 21.0)
                        || !closeEnough(gotTorque[idx], 22.0)
                        || !closeEnough(gotStiffness[idx], 23.0)
