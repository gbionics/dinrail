...
inline bool wait_for_multi_modes(yarp::dev::IControlMode* icmd, int expected, int joints[2], int rets[2])
{
    constexpr int maxAttempts = 20;
    constexpr double sleepSeconds = 0.001;
    bool b = false;
...
inline void test_single_joint(yarp::dev::IControlMode* icmd, yarp::dev::IAxisInfo* iinfo, int mode)
{
    ...
    CHECK(b);
    int ret=0;
    b = wait_for_single_mode(icmd, mode, &ret);
    CHECK(b);
    CHECK(mode==ret);
}

inline void test_all_joints(yarp::dev::IControlMode* icmd, yarp::dev::IAxisInfo* iinfo, int mode)
{
    REQUIRE(icmd != nullptr);
    ...
    CHECK(b);
    int rets[2] = {0,-0};
    b = wait_for_all_modes(icmd, mode, rets);
    CHECK(b);
    CHECK(mode == rets[0]);
    CHECK(mode == rets[1]);
}

inline void test_multi_joint(yarp::dev::IControlMode* icmd, yarp::dev::IAxisInfo* iinfo, int mode)
{
    REQUIRE(icmd != nullptr);
    REQUIRE(iinfo != nullptr);
    int ax=0; bool bax= false;
    ...
    CHECK(b);
    int rets[2] = { 0,-0 };
    b = wait_for_multi_modes(icmd, mode, joints, rets);
    CHECK(b);
    CHECK(mode == rets[0]);
    CHECK(mode == rets[1]);
}

inline void test_joint(yarp::dev::IControlMode* icmd, yarp::dev::IAxisInfo* iinfo, int mode)
{
    REQUIRE(icmd != nullptr);
    ...
    inline void exec_iControlMode_test_1(yarp::dev::IControlMode* icmd, yarp::dev::IAxisInfo* iinfo)
    {
        REQUIRE(icmd != nullptr);
        REQUIRE(iinfo != nullptr);
