...
bool DinRailControlBoardRemapper::attachAllUsingAxesNames(const PolyDriverList& polylist)
{
    ...
    for (const auto& axesName : axesNames)
    {
        ...
        if (std::find(subControlBoardsKeys.begin(), subControlBoardsKeys.end(), key)
            == subControlBoardsKeys.end())
        {
    ...
    for (size_t l = 0; l < axesNames.size(); l++)
    {
        ...
        remappedControlBoards.lut[l].subControlBoardIndex
            = subControlBoardKey2IndexInRemappedControlBoards[loc.subDeviceKey];
...
bool DinRailControlBoardRemapper::setPidReference(const PidControlTypeEnum& pidtype,
                                                  int j,
                                                  double ref)
{
    int off = (int)remappedControlBoards.lut[j].axisIndexInSubControlBoard;
...
bool DinRailControlBoardRemapper::setPidReferences(const PidControlTypeEnum& pidtype,
                                                   const double* refs)
{
    bool ret = true;
...
bool DinRailControlBoardRemapper::setPidErrorLimit(const PidControlTypeEnum& pidtype,
                                                   int j,
                                                   double limit)
{
    int off = (int)remappedControlBoards.lut[j].axisIndexInSubControlBoard;
...
bool DinRailControlBoardRemapper::setPidErrorLimits(const PidControlTypeEnum& pidtype,
                                                    const double* limits)
{
    bool ret = true;
...
bool DinRailControlBoardRemapper::getPidOutput(const PidControlTypeEnum& pidtype,
                                               int j,
                                               double* out)
{
    int off = (int)remappedControlBoards.lut[j].axisIndexInSubControlBoard;
...
bool DinRailControlBoardRemapper::getPidReference(const PidControlTypeEnum& pidtype,
                                                  int j,
                                                  double* ref)
{
    int off = (int)remappedControlBoards.lut[j].axisIndexInSubControlBoard;
...
bool DinRailControlBoardRemapper::getPidErrorLimit(const PidControlTypeEnum& pidtype,
                                                   int j,
                                                   double* limit)
{
    int off = (int)remappedControlBoards.lut[j].axisIndexInSubControlBoard;
...
bool DinRailControlBoardRemapper::getPidErrorLimits(const PidControlTypeEnum& pidtype,
                                                    double* limits)
{
    bool ret = true;
...
bool DinRailControlBoardRemapper::isPidEnabled(const PidControlTypeEnum& pidtype,
                                               int j,
                                               bool* enabled)
{
    int off = (int)remappedControlBoards.lut[j].axisIndexInSubControlBoard;
...
bool DinRailControlBoardRemapper::positionMove(const int n_joints,
                                               const int* joints,
                                               const double* refs)
{
    bool ret = true;
    ...
    selectedJointsBuffers.fillSubControlBoardBuffersFromArbitraryJointVector(refs,
                                                                             n_joints,
                                                                             joints,
        ...
        if (p->pos)
        {
            ok = p->pos
                     ->positionMove(selectedJointsBuffers.m_nJointsInSubControlBoard[ctrlBrd],
                                    ...
                                    selectedJointsBuffers.m_bufferForSubControlBoard[ctrlBrd]
                                        .data());
        } else
        {
            ok = false;
        ...
        if (p->pos)
        {
            ok = p->pos->getTargetPositions(allJointsBuffers.m_nJointsInSubControlBoard[ctrlBrd],
                                            allJointsBuffers.m_jointsInSubControlBoard[ctrlBrd]
                                                .data(),
                                            allJointsBuffers.m_bufferForSubControlBoard[ctrlBrd]
                                                .data());
        } else
        {
            ok = false;
...
bool DinRailControlBoardRemapper::getTargetPositions(const int n_joints,
                                                     const int* joints,
                                                     double* targets)
{
    bool ret = true;
    ...
    for (size_t ctrlBrd = 0; ctrlBrd < remappedControlBoards.getNrOfSubControlBoards(); ctrlBrd++)
    {
        ...
        {
            ok = p->pos
                     ->getTargetPositions(selectedJointsBuffers.m_nJointsInSubControlBoard[ctrlBrd],
    ...
    selectedJointsBuffers.fillArbitraryJointVectorFromSubControlBoardBuffers(targets,
                                                                             n_joints,
                                                                             joints,
                                                                             remappedControlBoards);
...
bool DinRailControlBoardRemapper::relativeMove(const double* deltas)
{
    ...
    allJointsBuffers.fillSubControlBoardBuffersFromCompleteJointVector(deltas,
                                                                       remappedControlBoards);
...
bool DinRailControlBoardRemapper::relativeMove(const int n_joints,
                                               const int* joints,
                                               const double* deltas)
{
    bool ret = true;
    ...
    selectedJointsBuffers.fillSubControlBoardBuffersFromArbitraryJointVector(deltas,
                                                                             n_joints,
                                                                             joints,
                                                                             remappedControlBoards);
...
bool DinRailControlBoardRemapper::setRefSpeeds(const int n_joints,
                                               const int* joints,
                                               const double* spds)
{
    bool ret = true;
    ...
    selectedJointsBuffers.fillSubControlBoardBuffersFromArbitraryJointVector(spds,
                                                                             n_joints,
                                                                             joints,
                                                                             remappedControlBoards);
...
bool DinRailControlBoardRemapper::setRefAccelerations(const int n_joints,
                                                      const int* joints,
                                                      const double* accs)
{
    bool ret = true;
    ...
    selectedJointsBuffers.fillSubControlBoardBuffersFromArbitraryJointVector(accs,
                                                                             n_joints,
                                                                             joints,
                                                                             remappedControlBoards);
    ...
    for (size_t ctrlBrd = 0; ctrlBrd < remappedControlBoards.getNrOfSubControlBoards(); ctrlBrd++)
    {
        ...
        {
            ok = p->pos
                     ->setRefAccelerations(selectedJointsBuffers.m_nJointsInSubControlBoard[ctrlBrd],
                                           selectedJointsBuffers.m_jointsInSubControlBoard[ctrlBrd]
                                               .data(),
                                           selectedJointsBuffers.m_bufferForSubControlBoard[ctrlBrd]
                                               .data());
...
bool DinRailControlBoardRemapper::getRefSpeeds(const int n_joints, const int* joints, double* spds)
{
    ...
    selectedJointsBuffers.fillArbitraryJointVectorFromSubControlBoardBuffers(spds,
                                                                             n_joints,
                                                                             joints,
                                                                             remappedControlBoards);
...
bool DinRailControlBoardRemapper::getRefAccelerations(const int n_joints,
                                                      const int* joints,
                                                      double* accs)
{
    bool ret = true;
    ...
    for (size_t ctrlBrd = 0; ctrlBrd < remappedControlBoards.getNrOfSubControlBoards(); ctrlBrd++)
    {
        ...
        {
            ok = p->pos
                     ->getRefAccelerations(selectedJointsBuffers.m_nJointsInSubControlBoard[ctrlBrd],
                                           selectedJointsBuffers.m_jointsInSubControlBoard[ctrlBrd]
                                               .data(),
                                           selectedJointsBuffers.m_bufferForSubControlBoard[ctrlBrd]
                                               .data());
    ...
    selectedJointsBuffers.fillArbitraryJointVectorFromSubControlBoardBuffers(accs,
                                                                             n_joints,
                                                                             joints,
                                                                             remappedControlBoards);
...
bool DinRailControlBoardRemapper::stop(const int n_joints, const int* joints)
{
    ...
    selectedJointsBuffers
        .fillSubControlBoardBuffersFromArbitraryJointVector(buffers.dummyBuffer.data(),
                                                            n_joints,
                                                            joints,
                                                            remappedControlBoards);
...
bool DinRailControlBoardRemapper::calibrateWholePart()
{
    if (!getCalibratorDevice())
    {
        for (size_t ctrlBrd = 0; ctrlBrd < remappedControlBoards.getNrOfSubControlBoards();
             ctrlBrd++)
        {
...
/* IControlCalibration */
bool DinRailControlBoardRemapper::calibrateAxisWithParams(
...
bool DinRailControlBoardRemapper::setCalibrationParameters(int j,
...
bool DinRailControlBoardRemapper::setRefTorques(const double* t)
{
    ...
    for (size_t ctrlBrd = 0; ctrlBrd < remappedControlBoards.getNrOfSubControlBoards(); ctrlBrd++)
    {
        ...
        {
            ok = p->iTorque
                     ->setRefTorques(allJointsBuffers.m_nJointsInSubControlBoard[ctrlBrd],
                                     allJointsBuffers.m_jointsInSubControlBoard[ctrlBrd].data(),
                                     allJointsBuffers.m_bufferForSubControlBoard[ctrlBrd].data());
        } else
        {
...
bool DinRailControlBoardRemapper::setRefTorques(const int n_joints,
                                                const int* joints,
                                                const double* t)
{
    bool ret = true;
    ...
    selectedJointsBuffers.fillSubControlBoardBuffersFromArbitraryJointVector(t,
                                                                             n_joints,
                                                                             joints,
                                                                             remappedControlBoards);
    ...
    for (size_t ctrlBrd = 0; ctrlBrd < remappedControlBoards.getNrOfSubControlBoards(); ctrlBrd++)
    {
        ...
        bool ok
            = p->iTorque
                  ->setRefTorques(selectedJointsBuffers.m_nJointsInSubControlBoard[ctrlBrd],
                                  selectedJointsBuffers.m_jointsInSubControlBoard[ctrlBrd].data(),
...
bool DinRailControlBoardRemapper::getMotorTorqueParams(int j,
...
bool DinRailControlBoardRemapper::setMotorTorqueParams(
    int j, const yarp::dev::MotorTorqueParameters params)
{
    int off = (int)remappedControlBoards.lut[j].axisIndexInSubControlBoard;
    size_t subIndex = remappedControlBoards.lut[j].subControlBoardIndex;
...
bool DinRailControlBoardRemapper::getCurrentImpedanceLimit(
    int j, double* min_stiff, double* max_stiff, double* min_damp, double* max_damp)
{
    ...
    if (p->iImpedance)
    {
        return p->iImpedance->getCurrentImpedanceLimit(off,
                                                       min_stiff,
                                                       max_stiff,
                                                       min_damp,
                                                       max_damp);
...
bool DinRailControlBoardRemapper::setSetPoint(
    int j, double pos, double vel, double torque, double stiffness, double damping)
{
    ...
    {
        return p->iImpedanceAllSetPointsControl
            ->setSetPoint(off, pos, vel, torque, stiffness, damping);
    }

    return false;
}

bool DinRailControlBoardRemapper::setSetPoints(
    ...
    const dinrail::VectorProxy<const double>::Ref damping)
{
    ...
    selectedJointsBuffers
        .fillSubControlBoardSetPointBuffersFromArbitraryJointVectors(pos.data(),
                                                                     vel.data(),
                                                                     torque.data(),
                                                                     stiffness.data(),
                                                                     damping.data(),
    ...
    const dinrail::VectorProxy<const double>::Ref damping)
{
    ...
    allJointsBuffers
        .fillSubControlBoardSetPointBuffersFromCompleteJointVectors(pos.data(),
                                                                    vel.data(),
                                                                    torque.data(),
                                                                    stiffness.data(),
                                                                    damping.data(),
    ...
    for (size_t ctrlBrd = 0; ctrlBrd < remappedControlBoards.getNrOfSubControlBoards(); ctrlBrd++)
    {
        ...
        const bool ok
            = p->iImpedanceAllSetPointsControl
                  ->setSetPoints(allJointsBuffers.m_jointsInSubControlBoard[ctrlBrd],
                                 ...
                                 allJointsBuffers
                                     .m_bufferForSubControlBoardSetPointStiffness[ctrlBrd],
                                 allJointsBuffers
                                     .m_bufferForSubControlBoardSetPointDamping[ctrlBrd]);
        ret = ret && ok;
    }
...
bool DinRailControlBoardRemapper::getSetPoint(
    int j, double& pos, double& vel, double& torque, double& stiffness, double& damping)
{
    const int off = static_cast<int>(remappedControlBoards.lut[j].axisIndexInSubControlBoard);
    const size_t subIndex = remappedControlBoards.lut[j].subControlBoardIndex;
    ...
    {
        return p->iImpedanceAllSetPointsControl
            ->getSetPoint(off, pos, vel, torque, stiffness, damping);
    }

    return false;
}

bool DinRailControlBoardRemapper::getSetPoints(
    const dinrail::VectorProxy<const int>::Ref jointIndeces,
    ...
    dinrail::VectorProxy<double>::Ref damping)
{
    ...
    selectedJointsBuffers
        .fillArbitraryJointVectorsFromSubControlBoardSetPointBuffers(pos.data(),
                                                                     vel.data(),
                                                                     torque.data(),
                                                                     stiffness.data(),
                                                                     damping.data(),
                                               ...
                                               dinrail::VectorProxy<double>::Ref damping)
{
    ...
    for (size_t ctrlBrd = 0; ctrlBrd < remappedControlBoards.getNrOfSubControlBoards(); ctrlBrd++)
    {
        ...
        const bool ok
            = p->iImpedanceAllSetPointsControl
                  ->getSetPoints(allJointsBuffers.m_jointsInSubControlBoard[ctrlBrd],
                                 allJointsBuffers.m_bufferForSubControlBoardSetPointPos[ctrlBrd],
                                 allJointsBuffers.m_bufferForSubControlBoardSetPointVel[ctrlBrd],
                                 allJointsBuffers.m_bufferForSubControlBoardSetPointTorque[ctrlBrd],
                                 allJointsBuffers
    ...
    allJointsBuffers
        .fillCompleteJointVectorsFromSubControlBoardSetPointBuffers(pos.data(),
                                                                    vel.data(),
                                                                    torque.data(),
                                                                    stiffness.data(),
                                                                    damping.data(),
...
bool DinRailControlBoardRemapper::getControlModes(int* modes)
{
    ...
    for (size_t ctrlBrd = 0; ctrlBrd < remappedControlBoards.getNrOfSubControlBoards(); ctrlBrd++)
    {
        ...
        {
            ok = p->iMode
                     ->getControlModes(allJointsBuffers.m_nJointsInSubControlBoard[ctrlBrd],
                                       allJointsBuffers.m_jointsInSubControlBoard[ctrlBrd].data(),
                                       allJointsBuffers
                                           .m_bufferForSubControlBoardControlModes[ctrlBrd]
                                           .data());
        } else
        {
            ok = false;
    ...
    allJointsBuffers.fillCompleteJointVectorFromSubControlBoardBuffers(modes,
                                                                       remappedControlBoards);
...
bool DinRailControlBoardRemapper::getControlModes(const int n_joints, const int* joints, int* modes)
{
    ...
    selectedJointsBuffers.fillArbitraryJointVectorFromSubControlBoardBuffers(modes,
                                                                             n_joints,
                                                                             joints,
                                                                             remappedControlBoards);
...
bool DinRailControlBoardRemapper::setControlModes(const int n_joints, const int* joints, int* modes)
{
    ...
    selectedJointsBuffers.fillSubControlBoardBuffersFromArbitraryJointVector(modes,
                                                                             n_joints,
                                                                             joints,
                                                                             remappedControlBoards);
...
bool DinRailControlBoardRemapper::setControlModes(int* modes)
{
    ...
    allJointsBuffers.fillSubControlBoardBuffersFromCompleteJointVector(modes,
                                                                       remappedControlBoards);
    ...
    for (size_t ctrlBrd = 0; ctrlBrd < remappedControlBoards.getNrOfSubControlBoards(); ctrlBrd++)
    {
        ...
        {
            ok = p->iMode
                     ->setControlModes(allJointsBuffers.m_nJointsInSubControlBoard[ctrlBrd],
                                       allJointsBuffers.m_jointsInSubControlBoard[ctrlBrd].data(),
                                       allJointsBuffers
                                           .m_bufferForSubControlBoardControlModes[ctrlBrd]
                                           .data());
...
bool DinRailControlBoardRemapper::setPositions(const int n_joints,
                                               const int* joints,
                                               const double* dpos)
{
    bool ret = true;
    ...
    selectedJointsBuffers.fillSubControlBoardBuffersFromArbitraryJointVector(dpos,
                                                                             n_joints,
                                                                             joints,
                                                                             remappedControlBoards);
...
bool DinRailControlBoardRemapper::getRefPositions(double* spds)
{
    ...
    for (size_t ctrlBrd = 0; ctrlBrd < remappedControlBoards.getNrOfSubControlBoards(); ctrlBrd++)
    {
        ...
        {
            ok = p->posDir
                     ->getRefPositions(allJointsBuffers.m_nJointsInSubControlBoard[ctrlBrd],
                                       allJointsBuffers.m_jointsInSubControlBoard[ctrlBrd].data(),
                                       allJointsBuffers.m_bufferForSubControlBoard[ctrlBrd].data());
        } else
        {
...
bool DinRailControlBoardRemapper::getRefPositions(const int n_joints,
                                                  const int* joints,
                                                  double* targets)
{
    bool ret = true;
    ...
    for (size_t ctrlBrd = 0; ctrlBrd < remappedControlBoards.getNrOfSubControlBoards(); ctrlBrd++)
    {
        ...
        {
            ok = p->posDir
                     ->getRefPositions(selectedJointsBuffers.m_nJointsInSubControlBoard[ctrlBrd],
                                       selectedJointsBuffers.m_jointsInSubControlBoard[ctrlBrd]
                                           .data(),
                                       selectedJointsBuffers.m_bufferForSubControlBoard[ctrlBrd]
                                           .data());
...
bool DinRailControlBoardRemapper::velocityMove(const int n_joints,
                                               const int* joints,
                                               const double* spds)
{
    bool ret = true;
...
bool DinRailControlBoardRemapper::getRefVelocities(double* vels)
{
    ...
    for (size_t ctrlBrd = 0; ctrlBrd < remappedControlBoards.getNrOfSubControlBoards(); ctrlBrd++)
    {
        ...
        {
            ok = p->vel
                     ->getRefVelocities(allJointsBuffers.m_nJointsInSubControlBoard[ctrlBrd],
                                        allJointsBuffers.m_jointsInSubControlBoard[ctrlBrd].data(),
                                        allJointsBuffers.m_bufferForSubControlBoard[ctrlBrd].data());
        } else
        {
...
bool DinRailControlBoardRemapper::getRefVelocities(const int n_joints,
                                                   const int* joints,
                                                   double* vels)
{
    bool ret = true;
    ...
    selectedJointsBuffers.fillArbitraryJointVectorFromSubControlBoardBuffers(vels,
                                                                             n_joints,
                                                                             joints,
                                                                             remappedControlBoards);
...
bool DinRailControlBoardRemapper::getInteractionModes(int n_joints,
                                                      int* joints,
                                                      yarp::dev::InteractionModeEnum* modes)
{
    ...
    for (size_t ctrlBrd = 0; ctrlBrd < remappedControlBoards.getNrOfSubControlBoards(); ctrlBrd++)
    {
        ...
        {
            ok = p->iInteract
                     ->getInteractionModes(selectedJointsBuffers.m_nJointsInSubControlBoard[ctrlBrd],
                                           selectedJointsBuffers.m_jointsInSubControlBoard[ctrlBrd]
                                               .data(),
                                           selectedJointsBuffers
                                               .m_bufferForSubControlBoardInteractionModes[ctrlBrd]
                                               .data());
        } else
        {
            ok = false;
...
bool DinRailControlBoardRemapper::getInteractionModes(yarp::dev::InteractionModeEnum* modes)
{
    ...
    for (size_t ctrlBrd = 0; ctrlBrd < remappedControlBoards.getNrOfSubControlBoards(); ctrlBrd++)
    {
        ...
        {
            ok = p->iInteract
                     ->getInteractionModes(allJointsBuffers.m_nJointsInSubControlBoard[ctrlBrd],
                                           allJointsBuffers.m_jointsInSubControlBoard[ctrlBrd]
                                               .data(),
                                           allJointsBuffers
                                               .m_bufferForSubControlBoardInteractionModes[ctrlBrd]
...
bool DinRailControlBoardRemapper::setInteractionModes(int n_joints,
                                                      int* joints,
                                                      yarp::dev::InteractionModeEnum* modes)
{
    bool ret = true;
    ...
    for (size_t ctrlBrd = 0; ctrlBrd < remappedControlBoards.getNrOfSubControlBoards(); ctrlBrd++)
    {
        ...
        bool ok
            = p->iInteract
                  ->setInteractionModes(selectedJointsBuffers.m_nJointsInSubControlBoard[ctrlBrd],
                                        selectedJointsBuffers.m_jointsInSubControlBoard[ctrlBrd]
                                            .data(),
                                        selectedJointsBuffers
                                            .m_bufferForSubControlBoardInteractionModes[ctrlBrd]
...
bool DinRailControlBoardRemapper::setInteractionModes(yarp::dev::InteractionModeEnum* modes)
{
    ...
    for (size_t ctrlBrd = 0; ctrlBrd < remappedControlBoards.getNrOfSubControlBoards(); ctrlBrd++)
    {
        ...
        bool ok
            = p->iInteract
                  ->setInteractionModes(allJointsBuffers.m_nJointsInSubControlBoard[ctrlBrd],
                                        allJointsBuffers.m_jointsInSubControlBoard[ctrlBrd].data(),
                                        allJointsBuffers
                                            .m_bufferForSubControlBoardInteractionModes[ctrlBrd]
                                            .data());
...
bool DinRailControlBoardRemapper::setRefCurrents(const int n_motor,
                                                 const int* motors,
                                                 const double* currs)
{
    bool ret = true;
    ...
    selectedJointsBuffers.fillSubControlBoardBuffersFromArbitraryJointVector(currs,
                                                                             n_motor,
                                                                             motors,
                                                                             remappedControlBoards);
...
bool DinRailControlBoardRemapper::setRefCurrents(const double* currs)
{
    ...
    allJointsBuffers.fillSubControlBoardBuffersFromCompleteJointVector(currs,
                                                                       remappedControlBoards);
    ...
    for (size_t ctrlBrd = 0; ctrlBrd < remappedControlBoards.getNrOfSubControlBoards(); ctrlBrd++)
    {
        ...
        bool ok
            = p->iCurr->setRefCurrents(allJointsBuffers.m_nJointsInSubControlBoard[ctrlBrd],
                                       allJointsBuffers.m_jointsInSubControlBoard[ctrlBrd].data(),
                                       allJointsBuffers.m_bufferForSubControlBoard[ctrlBrd].data());
        ret = ret && ok;
    }
