...
int main(int argc, char** argv)
{
    ...
    // By default given a device name, dinrail first tries to load a dinrail device if it can finds
    // it, otherwise it falls back to YARP device. By passing --only-use-yarp-device it is possible
    // to force dinrail to load the fakeMotionControl YARP device directly.
    ...
    if (!drv.open(opts))
    {
        std::cerr << "Failed to open device 'fakeMotionControl'.\n"
                  << "Hint: ensure the fakeMotionControl plugin is available in your YARP "
                     "installation, and that dinrail has been compiled with YARP support.\n";
        return 1;
    }
